...
export function renderMessage(feed, m, myName, options = {}) {
  ...
  // avatar and the name above the bubble are where people click first. A channel
  // post speaks for the channel, so it opens the channel's own card instead.
  const authorUsername = !m.channelName && /^[a-z0-9_]{3,24}$/i.test(String(m.username || '')) ? String(m.username) : '';
  const openAuthor = authorUsername
    ? () => (options.onOpenProfile || window.Segment?.openUser)?.(authorUsername)
    : (m.channelName && options.onOpenRoomProfile ? () => options.onOpenRoomProfile() : null);
  if (openAuthor) {
    for (const node of [el.querySelector('.avatar'), el.querySelector('.meta')]) {
      if (!node) continue;
      node.classList.add('author-link');
      if (authorUsername) node.title = `@${authorUsername}`;
      node.addEventListener('click', (e) => { e.stopPropagation(); openAuthor(); });
    }
  }
