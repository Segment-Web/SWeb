...
export function mountSettings(root, close, client, renderIdentity) {
  ...
  const home = () => {
    ...
    root.innerHTML = `<div class="settings-shell">
      ...
      <div class="settings-nav">
        ${[['profile','Профиль','Имя, фото и информация'],['privacy','Конфиденциальность','Кто видит данные профиля'],['appearance','Оформление','Темы, масштаб и анимации'],['chats','Чаты','Отправка и отображение'],['devices','Устройства','Активные сеансы и перенос ключей'],['storage','Данные и память','Локальные данные приложения'],['language','Язык и доступность','Русский язык и контраст'],['mods','Модификации','Безопасные дополнения интерфейса']].map(([id,title,desc]) => `<button class="settings-nav-item" data-page="${id}"><i>${icon(id)}</i><span><b>${title}</b><small>${desc}</small></span><em>›</em></button>`).join('')}
  ...
  const devices = () => {
    shell('Устройства','Активные сеансы аккаунта',`<div class="settings-stack"><div data-sessions class="settings-sessions"><div class="settings-session-loading">Загрузка…</div></div><div class="settings-card"><b>Связать новое устройство</b><p>Одноразовый код переносит ключи истории и черновики в зашифрованном виде.</p><button class="settings-secondary" data-make-link>Создать код</button><div class="device-link-code hidden" data-link-code><span></span><button data-copy-link>Копировать</button></div><div class="device-link-claim"><input data-claim-code placeholder="Код с другого устройства"><button data-claim-link>Подключить</button></div></div></div>`);
    ...
    root.querySelector('[data-claim-link]').onclick=async()=>{const full=root.querySelector('[data-claim-code]').value.trim();const at=full.lastIndexOf('.');if(at<1){toast('Неверный код');return;}try{const data=await api('/api/auth/device-links/claim',{method:'POST',body:JSON.stringify({token:full.slice(0,at)})});const payload=await openDevicePayload(data.payload,full.slice(at+1));client._adoptHistoryKeys(payload.historyKeys||{});client.historyKeyArchive=new Map(Object.entries(payload.historyKeyArchive||{}));client.storage.setHistoryKeyArchive?.(payload.historyKeyArchive||{});client.storage.setDrafts?.(payload.drafts||{});toast('Устройство подключено');}catch{toast('Код истёк или повреждён');}};
  };
