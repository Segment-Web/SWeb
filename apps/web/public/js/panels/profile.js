// Панель «Профиль» — отдельный блок: аватар, никнейм, статус и настройки.
//
// Как и остальные панели, самостоятельна: строит свой DOM, подписывается на
...
export function profilePanel(client) {
  return {
    ...
    label: 'Профиль',
    icon: '👤',
    hideable: true, // профиль можно скрыть к краю (но не удалить)
    weight: 0.7,
    mount(body) {
      body.innerHTML = `
        <div class="profile">
          ...
          <button class="profile-settings" title="Настройки" aria-label="Настройки">
      ...
      const renderSettings = () => {
        settings.innerHTML = `
          <div class="sheet-head">
            <b>Настройки</b>
            <button data-act="close" title="Закрыть">×</button>
          </div>
