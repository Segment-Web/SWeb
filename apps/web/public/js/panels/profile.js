...
function openProfileQrModal(user, root) {
  ...
  const place = (left = Number.parseFloat(dialog.style.left) || initialCenter.left, top = Number.parseFloat(dialog.style.top) || initialCenter.top) => {
    ...
    dialog.style.top = `${Math.max(halfHeight, Math.min(top, modal.clientHeight - halfHeight - 8))}px`;
  };
...
function mountAccountModal(modal, body) {
  ...
  const place = () => {
    ...
    const centerY = Math.min(innerHeight - dialogRect.height / 2 - margin, Math.max(dialogRect.height / 2, desiredY));
