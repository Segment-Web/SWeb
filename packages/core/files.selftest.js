...
globalThis.fetch = async (url, options = {}) => {
  if (options.method === 'POST') {
    ...
    const id = Buffer.alloc(32, ++capabilityCounter).toString('base64url');
    store.set(id, bytes);
    return { ok: true, status: 201, json: async () => ({ id, size: bytes.length }) };
  }
  const id = new URL(String(url), 'http://segment.test').pathname.split('/api/files/')[1];
...
const { ref, url: localUrl } = await client._sealDataUrl(dataUrl, 'room-test');
ok(/^[A-Za-z0-9_-]{43}$/.test(ref.fileId) && ref.key.length === 32 && ref.size === original.length, 'seal returns a valid capability reference');
...
const url = await client._fetchToUrl(ref, 'room-test');
...
const wire = await client._toWire(event, 'room-test');
...
await client._hydrateAttachment(received, 'room-test');
...
const vWire = await client._attachmentToWire({ kind: 'video', data: dataUrl, poster }, 'room-test');
...
await client._hydrateAttachment(vRecv, 'room-test');
