...
let persistedKit = null;
const identityStorage = { ...mkStorage(), getCryptoKit: async () => persistedKit, setCryptoKit: async (_id, kit) => { persistedKit = kit; } };
const identityA = new SegmentClient({ storage: identityStorage });
Object.assign(identityA.self, { id: 'identity-user', name: 'Identity' });
await identityA._ensureCrypto();
const firstIdentity = JSON.stringify(identityA.kit.bundle.idDh);
const identityReload = new SegmentClient({ storage: identityStorage });
Object.assign(identityReload.self, { id: 'identity-user', name: 'Identity' });
await identityReload._ensureCrypto();
ok(JSON.stringify(identityReload.kit.bundle.idDh) === firstIdentity, 'identity key survives a same-device reload');
const acceptedIds=identityReload.kit.bundle.opks.slice(0,2).map((item)=>item.id);let syncCalls=0;let refillBatch=[];
identityReload._roomsApi=async(_method,path,body)=>{if(path!=='/api/auth/device-prekeys')return{};syncCalls++;if(syncCalls===1)return{count:2,ids:acceptedIds};refillBatch=body.opks;return{count:8,ids:[...acceptedIds,...body.opks.map((item)=>item.id)]};};
await identityReload._syncOwnPreKeys();
ok(identityReload.kit.bundle.opks.length===8&&refillBatch.length===6,'offline OPK consumption is reconciled and replenished on reconnect');
let persistedPins = {};
const pinStorage = { ...mkStorage(), getSecureIdentityPins:async()=>structuredClone(persistedPins), setSecureIdentityPins:async(_id,pins)=>{persistedPins=structuredClone(pins);} };
const pinClient = new SegmentClient({ storage:pinStorage });
Object.assign(pinClient.self,{id:'pin-owner',name:'Pin owner'});
const securityEvents=[]; pinClient.on('security',(event)=>securityEvents.push(event));
const peerBundle=(deviceId,seed)=>({deviceId,idDh:[seed],idSign:[seed+1]});
await pinClient._addPeer({id:'peer-1',userId:'peer-user',name:'Peer',bundle:peerBundle('device-1',1)});
await pinClient._addPeer({id:'peer-2',userId:'peer-user',name:'Peer',bundle:peerBundle('device-2',3)});
await pinClient._addPeer({id:'peer-forged',userId:'peer-user',name:'Peer',bundle:peerBundle('device-1',9)});
ok(securityEvents.some((event)=>event.code==='NEW_DEVICE_ADDED')&&securityEvents.some((event)=>event.code==='IDENTITY_KEY_CHANGED')&&!pinClient.peers.has('peer-forged'),'TOFU warns on a new device and blocks replacement of a known device');
ok(Boolean(persistedPins['peer-user']?.devices?.['device-2']),'TOFU device set persists through secure storage');
...
B._applyEvent(secureRoom, { kind: 'message', message: { id: 'history-spoof', name: 'Admin', color: '#fff" onmouseover="alert(1)', text: 'history' } }, { id: 'deleted-user', history: true });
const sanitizedHistory = B._messageById(secureRoom, 'history-spoof');
ok(sanitizedHistory?.name === 'Удалённый аккаунт' && sanitizedHistory.color === '#7f8c9d', 'history backfill never trusts embedded author presentation');
...
ok(JSON.stringify(await senderA.key.export()) !== JSON.stringify(await senderB.key.export()), 'different rooms use different sender chains');
const formerMemberView = SenderKeyView.from(await senderA.key.export());
