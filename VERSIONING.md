...
The current version must match in `VERSION`, the root `package.json`, every workspace package and `package-lock.json`. Each published release receives a matching changelog entry and Git tag such as `v0.0.1`.
